...
export const ExcludedCommoditiesModal: React.FC<ExcludedCommoditiesModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div
        className="relative w-full max-w-2xl max-h-[85vh] bg-white dark:bg-[#0B0F17] border border-slate-200 dark:border-white/[0.15] rounded-2xl shadow-2xl flex flex-col overflow-hidden text-slate-800 dark:text-slate-200"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
        ...
        {/* Content list */}
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-3 bg-white dark:bg-transparent">
          <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-xs text-amber-900 dark:text-amber-200 flex items-start space-x-2.5">
          ...
          <div className="space-y-2.5 pt-1">
            {EXCLUDED_COMMODITIES.map((comm, idx) => (
              <div
                key={idx}
                className="p-3 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.07] hover:border-slate-300 dark:hover:border-white/[0.15] transition-all"
              >
                <div className="text-xs font-bold text-slate-900 dark:text-slate-100 flex items-center space-x-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-rose-500 dark:bg-rose-400" />
                  <span>{comm.name}</span>
                </div>
                <div className="text-[11px] text-slate-600 dark:text-slate-400 pl-3.5 mt-1 leading-relaxed">
                  {comm.reason}
                </div>
              </div>
            ))}
        ...
        {/* Footer */}
        <div className="p-4 sm:px-6 border-t border-slate-200 dark:border-white/[0.1] bg-slate-50 dark:bg-[#0F1420] flex items-center justify-end">
          <button
            type="button"
            onClick={onClose}
