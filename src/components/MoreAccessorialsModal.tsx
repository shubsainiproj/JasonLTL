...
export const MoreAccessorialsModal: React.FC<MoreAccessorialsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div
        className="relative w-full max-w-3xl max-h-[90vh] bg-[#0B0F17] border border-white/[0.15] rounded-2xl shadow-2xl flex flex-col overflow-hidden text-slate-200"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
        ...
        <div className="p-4 sm:px-6 border-b border-white/[0.08] bg-[#0B0F17] space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="glass-input w-full pl-10 pr-10 py-2.5 rounded-xl text-sm placeholder-slate-500 font-medium"
            />
            {searchQuery && (
              <button
                type="button"
        ...
        <div className="px-4 sm:px-6 py-2.5 bg-amber-500/10 border-b border-amber-500/20 flex items-center space-x-2 text-xs text-amber-200">
                ...
                return (
                  <label
                    ...
                    className={`flex items-start space-x-3 p-3 rounded-xl border transition-all cursor-pointer select-none ${
                      isSelected
                        ? 'bg-yellow-400/15 border-yellow-400/50 text-white shadow-sm'
                    ...
                    <div className="flex-1 min-w-0 text-xs">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {item.requiresEmailQuote && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-300 bg-amber-500/20 px-1.5 py-0.2 rounded border border-amber-500/30">
        ...
        {/* Footer */}
        <div className="p-4 sm:px-6 border-t border-white/[0.1] bg-[#0F1420] flex items-center justify-between">
          <div className="text-xs text-slate-400">
            <span className="text-[#FACC15] font-bold">{selectedIds.length}</span> accessorial{selectedIds.length === 1 ? '' : 's'} selected
          </div>
          <button
            type="button"
            onClick={onClose}
